...
class Procedure(ABCWidget):
    ...
    def change_ContentsClickHis(self, ContentsClickHis:int, ContentsClickHis_index:int):
        ...
        """
        current_title = self.inmem.ProcedureHis[self.procedure_name]['SequenceTitleClick']
        self.inmem.ProcedureHis[self.procedure_name]['ContentsClickHis'][current_title][ContentsClickHis_index] = ContentsClickHis
    ...
    def Rule_1_Change_SequenceTitleCondHis(self):
        """Rule 1 : 만약 ContentsClickHis 가 2 (불만족) 이라면 SequenceTitleCondHis 를 3으로 업데이트 없다면 0"""
        current_title = self.inmem.ProcedureHis[self.procedure_name]['SequenceTitleClick']
        all_step_check = any([True if cond == 2 else False for cond in self.inmem.ProcedureHis[self.procedure_name]['ContentsClickHis'][current_title]])
        return all_step_check
    
    def Rule_2_CheckComplete(self):
        """Rule 2 : 만약 모든 ContentsClickHis 가 1 (완료) 이라면 ProcedureComplete BTN Enable"""
        current_title = self.inmem.ProcedureHis[self.procedure_name]['SequenceTitleClick']
        all_step_check = all([True if cond == 1 else False for cond in self.inmem.ProcedureHis[self.procedure_name]['ContentsClickHis'][current_title]])
...
class ProcedureSequence(ABCWidget):
    def __init__(self, parent):
        ...
        lay.addWidget(ProcedureSequenceWidget(self, 'ProcedureSequenceTitleAl',  'ProcedureSequenceTitleCondAl', '경보 및 증상'))
        lay.addWidget(ProcedureSequenceWidget(self, 'ProcedureSequenceTitleAu',  'ProcedureSequenceTitleCondAu', '자동 동작 사항'))
        lay.addWidget(ProcedureSequenceWidget(self, 'ProcedureSequenceTitleUr',  'ProcedureSequenceTitleCondUr', '긴급 조치 사항'))
        lay.addWidget(ProcedureSequenceWidget(self, 'ProcedureSequenceTitleFo',  'ProcedureSequenceTitleCondFo', '후속 조치 사항'))
        lay.setSpacing(15)
        lay.addStretch(1)
...
class ProcedureContents(ABCWidget):
    ...
    def dis_update(self, SequenceTitleClick:str, ContentsClickHis:dict, Contents:dict):
        ...
        self.widgets = {i: Procedure_Content(self, Contents_inTitle[i], ContentsClickHis_inTitle[i], i) for i in range(len(ContentsClickHis_inTitle))}
...
class Procedure_Content_Check(ABCPushButton):
    ...
    def is_clicked(self):
        ...
        self.inmem.widget_ids['Procedure'].change_ContentsClickHis(self.ContentsClickHis_inTitle, self.ContentsClickHis_inTitle_index)
        self.dis_update()
