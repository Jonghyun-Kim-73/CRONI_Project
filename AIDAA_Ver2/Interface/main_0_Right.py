...
class MainParaArea1(QTableWidget):
    ...
    def update_procedure(self):  # 추후 비정상 절차서 이름 앞에 라벨 제거
        ...
        ai_ref = {i: {'name': diagnosis_convert_text[int(ai_proc.iloc[i][0])],
                      'em': len(ab_pro[diagnosis_convert_text[int(ai_proc.iloc[i][0])]]['긴급 조치 사항']),
                      'if_prob': f'{symp_satis[i]}/{self.shmem.get_pro_symptom_count(diagnosis_convert_text[int(ai_proc.iloc[i][0])])}',
                      'ai_prob': round(ai_proc.iloc[i][1] * 100, 2)} for i in range(len(ai_result))}
        for i in range(5):
...
class MainParaArea3(QGroupBox):
    ...
    def check_btn_press(self):
        ...
        if self.selected_ab != '':
            ...
            self.gb.setTitle(
                f"Symptom Check [{sum(self.symp_satis)}/{len(ab_pro[self.selected_ab]['경보 및 증상'])}]")  # 테이블 클릭 시 비정상 절차서의 "경보 및 증상" 요건의 개수를 반영
...
class MainParaArea3_1(QTableWidget):
    ...
    def update_shap(self):
        ...
        if Flag.call_bottom_name != '':
            ...
            temp2 = pd.DataFrame([temp1.index, prob], index=['describe', 'probability']).T.sort_values(by='probability',
                                                                                                       ascending=False,
                                                                                                       axis=0).reset_index(
                drop=True)

            [self.add_shap(i, f"{temp2['describe'][i]}",
                           f"<span style=\"color:#0000ff;\" >{temp2['probability'][i]}%</span>") for i in
             range(20)]  # 추후 색상 입히기
